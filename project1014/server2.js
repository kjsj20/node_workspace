...
Get: 가저오다 (서버로부터 컨텐츠를 가져올때 사용되는 요청 방식)
...
POST: 보내다는 의미 (클라이언트가 서버에 데이터를 전송할때 사용하는 방식)
        HTTP 프로토콜의 body를 통해 데이터를 전송하기 때문에 마치 편지지에 데이터를 실어서 보내는것과 같다. 봉투 안에 담아서 보내기 때문에 데이터가 노출되지 않는다.
...
var server = http.createServer(function(request, response){
    if(request.method == "GET"){
        ...
        response.end("클라이언트가 GET방식으로 요청했어요~");

        // get 방식으로 전달된 파라미터 받기!!
        var urlJson = url.parse(request.url, true); // true 매개변수를 넘기면, 파라미터 정보를 json으로 반환
        ...
        response.writeHead(200, {"Content-Type":"text/html;charset=utf-8"});
        response.end("클라이언트가 POST방식으로 요청했어요~");
        // bdoy로 전송되 데이터는 url 분석막으로는 해결이 안된다..
        // post방식으로 전달된 데이터를 받기 위한 이벤트를 감지해보자!!
        request.on("data", function(param){
            // var postParam = url.parse(new String(param), true);
