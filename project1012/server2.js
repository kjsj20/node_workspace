...
// 서버는 클라이언트 요청이 들어오면, 반드시 응답을 해야 한다../
// http 메카니즘이다.. 만일 응답을 안하면?? 클라이언트는 무한정 서버의 응답을 기다리므로
// 무한대기상태에 빠진다...
// 서버객체를 생성
// 서버 객체 생성시, 요청정보와 응답정보를 구성 할 수 있는 request, response객체가 매개변수로
// 전달될 수 있다.
var server = http.createServer(function(requset, response){
    ...
    // HTTP 상태코드 중 200은 정상 처리를 의미..(즉 서버에서 클라이언트의 요청으로 정상적으로)
    // 처리했다는 상태코드) 누가 정한건가? W3C 표준에 의해 정해진 것임..
